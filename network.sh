...
CHAINCODE_IDENTITY_REGISTER=identity-register
...
function generateOrdererDockerCompose() {
    echo "Creating orderer docker compose yaml file with $DOMAIN, $ORG1, $ORG2, $ORG3, $ORG4, $ORG5, $ORG6, $DEFAULT_ORDERER_PORT, $DEFAULT_WWW_PORT"
    ...
    sed -e "s/DOMAIN/$DOMAIN/g" -e "s/CLI_EXTRA_HOSTS/$cli_extra_hosts/g" -e "s/ORDERER_PORT/$DEFAULT_ORDERER_PORT/g" -e "s/WWW_PORT/$DEFAULT_WWW_PORT/g" -e "s/ORG1/$ORG1/g" -e "s/ORG2/$ORG2/g" -e "s/ORG3/$ORG3/g" -e "s/ORG4/$ORG4/g" -e "s/ORG5/$ORG5/g" -e "s/ORG6/$ORG6/g" ${compose_template} | awk '{gsub(/\[newline\]/, "\n")}1' > ${f}
}

function generateOrdererArtifacts() {
    echo "Creating orderer yaml files with $DOMAIN, $ORG1, $ORG2, $ORG3, $ORG4, $ORG5, $ORG6, $DEFAULT_ORDERER_PORT, $DEFAULT_WWW_PORT"

    f="ledger/docker-compose-$DOMAIN.yaml"

    # replace in configtx
    sed -e "s/DOMAIN/$DOMAIN/g" -e "s/ORG1/$ORG1/g" -e "s/ORG2/$ORG2/g" -e "s/ORG3/$ORG3/g" -e "s/ORG4/$ORG4/g" -e "s/ORG5/$ORG5/g" -e "s/ORG6/$ORG6/g" artifacts/configtxtemplate.yaml > artifacts/configtx.yaml
    ...
    sed -e "s/\DOMAIN/$DOMAIN/g" -e "s/\ORG1/$ORG1/g" -e "s/\ORG2/$ORG2/g" -e "s/\ORG3/$ORG3/g" -e "s/\ORG4/$ORG4/g" -e "s/\ORG5/$ORG5/g" -e "s/\ORG6/$ORG6/g" -e "s/^\s*\/\/.*$//g" artifacts/network-config-template.json > artifacts/network-config.json
    ...
    for channel_name in identity vaccination-us medi-council-us vaccination-mx medi-council-mx
    do
...
function installIdentityRegister(){
  org=$1
  for chaincode_name in ${CHAINCODE_IDENTITY_REGISTER}
  do
    installChaincode ${org} ${chaincode_name}
  done
}
...
function installMediCouncilRegister(){
  org=$1
  for chaincode_name in ${CHAINCODE_MEDI_COUNCIL_REGISTER}
  do
    installChaincode ${org} ${chaincode_name}
  done
}
...
if [ "${MODE}" == "up" -a "${ORG}" == "" ]; then
  ...
  for org in ${DOMAIN} ${ORG1} ${ORG2} ${ORG3} ${ORG4} ${ORG5} ${ORG6}
  do
    dockerComposeUp ${org}
  done
elif [ "${MODE}" == "api-up" -a "${ORG}" == "" ]; then
  for org in ${ORG1} ${ORG2} ${ORG3} ${ORG4} ${ORG5} ${ORG6}
  do 
  ...
  generatePeerArtifacts ${ORG5} 4004 8085 11054 11051 11053 11056 11058
  generatePeerArtifacts ${ORG6} 4005 8086 12054 12051 12053 12056 12058
